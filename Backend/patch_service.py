...
# Add import for timezone
if "from datetime import date, datetime" in content:
    content = content.replace("from datetime import date, datetime", "from datetime import date, datetime, timezone")

# Add import for ProcesamientoLog
if "from modules.transformacion.model.ProcesamientoLog import ProcesamientoLog" not in content:
...
pattern = r"    def process_raw_data\(self, limite: int = 1000, forzar_reproceso: bool = False\) -> Dict\[str, int\]:.*?(?=    # ──────────────────────────────────────────────────────────────\n    # Detección de anomalías)"
